...
export default function ClientContractCreate() {
  ...
  const { createClientContract, uploadContractPdf } =
    useClientContracts();
  ...
  const onSubmitForm = async (data: ClientContractFormInputs) => {
    ...
    if (data.mediaId?.trim()) {
      payload.mediaId = data.mediaId.trim();
    }
    if (data.drawingMediaId?.trim()) {
      payload.drawingMediaId = data.drawingMediaId.trim();
    }
    await createClientContract(payload);
  };
